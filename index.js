...
async function bnb(path) {
  try {
    const r = await fetch(`${BINANCE}${path}`, { headers: { Accept: "application/json" } });
    if (!r.ok) { console.log(`Binance ${r.status} ${path}`); return null; }
    return await r.json();
  } catch(e) { console.log("bnb err:", e.message); return null; }
}

async function getAllSymbols() {
  const d = await bnb("/fapi/v1/ticker/24hr");
  if (!Array.isArray(d)) { console.log("getAllSymbols gagal"); return []; }
  return d
...
async function getKlines(symbol, interval) {
  const d = await bnb(`/fapi/v1/klines?symbol=${symbol}&interval=${interval}m&limit=200`);
  if (!Array.isArray(d)) return null;
  return d.map(r => ({ t:+r[0], o:+r[1], h:+r[2], l:+r[3], c:+r[4], v:+r[5] }));
}

async function getFunding(symbol) {
  const d = await bnb(`/fapi/v1/fundingRate?symbol=${symbol}&limit=1`);
  if (!Array.isArray(d) || !d.length) return { rate: 0, bias: "neutral" };
  const rate = parseFloat(d[0].fundingRate);
...
async function getCurrentPrice(symbol) {
  const d = await bnb(`/fapi/v1/ticker/price?symbol=${symbol}`);
